    ...
    async initPlayer() {
      try {
        ...
        if (!window.SharedArrayBuffer) {
          this.error = 'SharedArrayBuffer 不可用。\n请在 vite.config.js 或 vue.config.js 中配置 CORS 头：\n' +
        ...
        // 动态导入 SDK
        const sdkPath = this.sdkPath.endsWith('/') ? this.sdkPath : this.sdkPath + '/';
        
        // 尝试从 SDK 目录加载
        let sdk;
        try {
          sdk = await import(/* @vite-ignore */ sdkPath + 'VVCPlayerSDK.mjs');
        } catch (e) {
          // 如果失败，尝试相对导入
          sdk = await import('./VVCPlayerSDK.mjs');
        }
        
        const { createVVCPlayer, setSDKBasePath } = sdk;
        setSDKBasePath(sdkPath);

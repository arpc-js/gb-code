...
function updateMuteIcon(muted) {
  if (muted) {
    btnMute.classList.remove('bi-volume-up-fill');
    btnMute.classList.add('bi-volume-mute-fill');
  } else {
    btnMute.classList.remove('bi-volume-mute-fill');
    btnMute.classList.add('bi-volume-up-fill');
  }
}

function showVolumeControl(show) {
...
btnPlayUrl.onclick = playFromUrl;
...
function playFromUrl() {
  ...
  document.getElementById("videoTitle").innerText = url.split('/').pop();
  const repeat = loopPlayback ? 0 : document.getElementById("repeat").value;
  player.play(url, repeat);
}
...
async function findAppPath() {
  const tryPaths = [document.location.href.match(/.*\//)[0] + 'bin/release-static/', document.location.href.match(/.*\//)[0] + 'bin/', '/install/bin/'];
...
function handleMetadata(data) {
  ...
  // Show volume control if audio is available
  if (player.hasAudio) {

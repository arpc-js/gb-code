  ...
  <div class="vvc-player-container">
    ...
    <div class="url-input-bar">
      <input 
        type="text" 
        v-model="videoUrl" 
        placeholder="输入 H.266 视频 URL (.mp4 或 .266)"
        class="url-input"
        @keyup.enter="playVideo"
      />
      <button class="play-btn" @click="playVideo" :disabled="!ready || !videoUrl">
        播放
      </button>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      statusText: '初始化中...',
      error: null,
      videoUrl: this.src || 'http://156.238.240.143:8080/output.mp4'  // 默认测试视频
    };
  },

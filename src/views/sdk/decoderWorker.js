...
// 延迟加载库，等待 init 消息设置路径
let libBasePath = '';
let libsLoaded = false;

function loadLibs(basePath) {
  if (libsLoaded) return;
  libBasePath = basePath || '';
  try {
    importScripts(libBasePath + 'lib/mp4box.all.min.js');
  } catch (e) {
    // 如果失败，尝试从 SDK 目录加载
    try {
      importScripts(libBasePath + 'mp4box.all.min.js');
    } catch (e2) {
      importScripts('lib/mp4box.all.min.js');
    }
  }
  try {
    importScripts(libBasePath + 'AsyncHelpers.js');
  } catch (e) {
    importScripts('AsyncHelpers.js');
  }
  libsLoaded = true;
  // 库加载后初始化 continueDecoding
  if (!continueDecoding) {
    continueDecoding = new AsyncWaitCondition();
  }
}
...
try {
  ...
  // 将在 init 消息中加载
  console.log('Libraries will be loaded on init');
}
...
let continueDecoding = null; // 延迟初始化
...
async function decodeMP4(decInstance, data, segmented) {
  ...
  const AUDIO_BATCH_SIZE = 10; // Send audio in batches for smoother playback
  
  // Get AAC codec specific config from track description
    ...
    try {
      ...
      if (entry && entry.esds && entry.esds.esd && entry.esds.esd.descs) {
        for (let desc of entry.esds.esd.descs) {
          if (desc.tag === 0x05 && desc.data) { // DecoderSpecificInfo
            return new Uint8Array(desc.data);
          }
        }
      }
      // Fallback: try to get from mp4a box
  ...
  // Process audio samples up to a given video timestamp
  const process_audio_samples_until = function (trak, audioTrackInfo, videoTimeSec) {
    if (!trak || trak.nextSample >= trak.samples.length) {
      return false;
    }
    ...
    while (trak.nextSample < trak.samples.length) {
      ...
      // Only process audio samples that are before or at the current video time + buffer
      if (audioTimeSec > videoTimeSec + 1.0) {
        break; // Don't advance, wait for video to catch up
      }
    ...
    // Flush remaining samples if we have some
  ...
  // Track current video time for audio sync
    ...
    while (trak.nextSample < trak.samples.length) {
      ...
      while (start < sample.data.byteLength && !exitDecoder) {
        ...
        if (frameHandle.frame) {
          ...
          // Update current video time and process corresponding audio
  ...
  await process_samples(extractTrak);
  // Flush remaining audio samples at the end
  if (audioTrak && audioTrack) {
...
onmessage = async function (e) {
  ...
  if (data.cmd === 'init') {
    // 加载库文件
    if (!libsLoaded) {
      loadLibs(data.libPath || '');
    }
    
    // 清理路径，避免重复斜杠
    let appPath = (data.appPath || '').replace(/\/+$/, '');
    let scriptUrl = appPath + "/vvdecapp.js";

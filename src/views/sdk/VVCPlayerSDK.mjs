...
export async function createVVCPlayer(canvas, options = {}) {
  ...
  if (typeof SharedArrayBuffer === 'undefined') {
    throw new Error('SharedArrayBuffer 不可用。请检查浏览器是否支持，以及 CORS 头是否配置正确。');
  }
  ...
  // 固定使用 /sdk/ 路径（public/sdk/ 目录）
  const sdkPath = '/sdk/';
  console.log('SDK path:', sdkPath);

  // 从 public/sdk/ 加载 VVCPlayer
  const { default: VVCPlayer } = await import(/* @vite-ignore */ sdkPath + 'VVCPlayer.mjs');
  ...
  const playerOptions = {
    workerPath: sdkPath + 'decoderWorker.js',
    libPath: sdkPath,
  };

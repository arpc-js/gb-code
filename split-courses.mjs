...
// 确保目录存在
mkdirSync('src/mock/java', { recursive: true })
mkdirSync('src/mock/frontend', { recursive: true })
...
// 写入文件
writeFileSync('src/mock/java/index.ts', javaContent)
writeFileSync('src/mock/frontend/index.ts', frontendContent)
...
export async function getCoursesByPath(pathId: string): Promise<Course[]> {
  if (pathId === 'java') {
    const { javaCourses } = await import('./java/index.ts')
    return javaCourses
  } else if (pathId === 'frontend') {
    const { frontendCourses } = await import('./frontend/index.ts')
...
console.log('Done! Courses split and main file updated.')
